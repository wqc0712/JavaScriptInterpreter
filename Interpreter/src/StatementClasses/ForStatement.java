...
public class ForStatement extends Statement {
    ...
    public boolean executeStatement(){
        ...
        ExpressionStatement es_operate = new ExpressionStatement(index_operate);
        try{
            Value value_init = es_init.executeExpression(index_init);
            Value value_judge = ex_judge.executeExpression(index_judge);
            Value value_operate = es_operate.executeExpression(index_operate);
            ...
            else if(value_operate.getType()!=4){
                throw new Exception("for括号内操作语句必须为double类型");
            }
